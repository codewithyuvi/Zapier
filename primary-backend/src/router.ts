...
export const appRouter = router({
  ...
  getAvailableTriggers: publicProcedure.query(async () => {
    const trigger = await db.select().from(availableTriggers);
    return trigger;
  }),

  getAvailableActions: publicProcedure.query(async () => {
    const action = await db.select().from(availableActions);
    return action;
  }),

  getZaps: protectedProcedure.query(async ({ ctx }) => {
  ...
  getZapRuns: protectedProcedure.query(async ({ ctx }) => {

...
export default function Dashboard() {
  const mockZaps = [
    { id: 1, name: "Webhook to Slack", status: "Active", trigger: "Webhook", action: "Slack" },
    { id: 2, name: "New Lead to Email", status: "Paused", trigger: "Form", action: "Email" },
  ];
              ...
              <div key={zap.id} className="glass rounded-xl p-5 flex items-center justify-between hover:bg-white/[0.02] transition-colors group cursor-pointer border border-white/5 hover:border-white/10">
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex items-center gap-2 w-24">
                    <div className={`w-2 h-2 rounded-full ${zap.status === 'Active' ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.4)]' : 'bg-gray-500'}`}></div>
                    <span className="text-sm text-gray-400 font-medium">{zap.status}</span>
                  </div>
                  
                  {/* Title & Flow */}
                  <div>
                    <h3 className="font-medium text-lg mb-1">{zap.name}</h3>
                    <div className="flex items-center gap-2 text-sm text-gray-400">
                      <span className="px-2 py-0.5 rounded-md bg-white/5 border border-white/5">{zap.trigger}</span>
                      <ArrowRight className="w-3 h-3" />
                      <span className="px-2 py-0.5 rounded-md bg-white/5 border border-white/5">{zap.action}</span>
                    </div>
                  </div>
                </div>
